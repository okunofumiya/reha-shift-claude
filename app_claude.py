...
def _calculate_penalty_breakdown(shifts_values, params):
    ...
    for d in params['sundays']:
        pt_on = sum(shifts_values.get((s, d),0) for s in params['pt_staff']); ot_on = sum(shifts_values.get((s, d),0) for s in params['ot_staff']); st_on = sum(shifts_values.get((s, d),0) for s in params['st_staff'])
    ...
    breakdown['S3: 外来同時休'] = round(sum(max(0, sum(1 - shifts_values.get((s, d),0) for s in params['gairai_staff']) - 1) * 10 for d in params['days']))
    breakdown['S4: 準希望休(△)'] = round(sum(shifts_values.get((s, d),0) for s, dates in params['requests_tri'].items() for d in dates) * params['tri_penalty_weight'])
    kaifukuki_penalty = 0
    for d in params['days']:
        if sum(shifts_values.get((s, d),0) for s in params['kaifukuki_pt']) == 0: kaifukuki_penalty += 5
        if sum(shifts_values.get((s, d),0) for s in params['kaifukuki_ot']) == 0: kaifukuki_penalty += 5
    ...
    for d in params['weekdays']:
        provided_units = sum(shifts_values.get((s, d),0) * int(params['staff_info'][s]['1日の単位数']) * (0.5 if s in params['requests_half'].get(s, []) and d in params['requests_half'][s] else 1.0) for s in params['staff'])
...
def solve_three_patterns(staff_df, requests_df, year, month, 
                         ...
                         unit_penalty_multiplier=5.0):  # 平滑化強度パラメータを追加
    ...
    def build_model(add_distance_constraint=False, base_solution=None, high_flat_penalty=False):
        ...
        # ★★★ 平滑化の強化 ★★★
        # ペナルティ重みを大幅に増加（UIの倍率も適用）
        unit_penalty_weight = round(10 * unit_penalty_multiplier) if not high_flat_penalty else round(20 * unit_penalty_multiplier)
        staff_penalty_weight = round(5 * unit_penalty_multiplier) if not high_flat_penalty else round(15 * unit_penalty_multiplier)
        
        # より正確な平均値計算
        expected_work_days = {}
        for s in staff:
            num_paid = len(requests_paid.get(s, []))
            num_special = len(requests_special.get(s, []))
            expected_work_days[s] = num_days - 9 - num_paid - num_special
        
        # 平日の期待総単位数を計算
        total_expected_units = 0
        for s in staff:
            if expected_work_days[s] > 0:
                weekday_work_days = expected_work_days[s] * len(weekdays) / num_days
                total_expected_units += int(staff_info[s]['1日の単位数']) * weekday_work_days
        
        avg_residual_units = (total_expected_units - sum(event_units.values())) / len(weekdays) if weekdays else 0
        
        # 最小出勤人数の目安（全職員の40%）
        min_staff_per_day = max(5, len(staff) * 0.4)
        
        job_types = {'PT': pt_staff, 'OT': ot_staff, 'ST': st_staff}
        for job, members in job_types.items():
            if not members: continue
            avg_work_days = sum(expected_work_days[s] for s in members)
        ...
        for d in weekdays:
            # 提供単位数の計算
            provided_units = sum(shifts[(s, d)] * int(staff_info[s]['1日の単位数']) * (0.5 if s in st.session_state.requests_half.get(s, []) and d in st.session_state.requests_half[s] else 1.0) for s in staff)
            event_unit = event_units.get(d, 0)
            residual_units = model.NewIntVar(-2000, 2000, f'r_{d}')
            model.Add(residual_units == provided_units - event_unit)
            
            diff = model.NewIntVar(-2000, 2000, f'u_d_{d}')
            model.Add(diff == residual_units - round(avg_residual_units))
            abs_diff = model.NewIntVar(0, 2000, f'a_u_d_{d}')
            model.AddAbsEquality(abs_diff, diff)
            penalties.append(unit_penalty_weight * abs_diff)
            
            # ★ 極端に少ない場合の追加ペナルティ
            threshold = round(avg_residual_units * 0.7)
            under_threshold = model.NewBoolVar(f'under_{d}')
            model.Add(residual_units < threshold).OnlyEnforceIf(under_threshold)
            model.Add(residual_units >= threshold).OnlyEnforceIf(under_threshold.Not())
            penalties.append(100 * under_threshold)  # 強い追加ペナルティ
            
            # ★ 最小出勤人数のソフト制約
            num_working = sum(shifts[(s, d)] for s in staff)
            min_violation = model.NewIntVar(0, len(staff), f'min_v_{d}')
            model.Add(min_violation >= round(min_staff_per_day) - num_working)
            penalties.append(50 * min_violation)
        
        # ★ 連続する日の差を抑制
        for i in range(len(weekdays) - 1):
            d1, d2 = weekdays[i], weekdays[i + 1]
            units_d1 = sum(shifts[(s, d1)] * int(staff_info[s]['1日の単位数']) for s in staff)
            units_d2 = sum(shifts[(s, d2)] * int(staff_info[s]['1日の単位数']) for s in staff)
            
            consecutive_diff = model.NewIntVar(-1000, 1000, f'cd_{d1}_{d2}')
            model.Add(consecutive_diff == units_d1 - units_d2)
            abs_consecutive_diff = model.NewIntVar(0, 1000, f'acd_{d1}_{d2}')
            model.AddAbsEquality(abs_consecutive_diff, consecutive_diff)
    ...
    with st.spinner(f"パターン2 (パターン1と{min_distance_N}マス以上違う解) を探索中..."):
        ...
        solver2 = cp_model.CpSolver(); solver2.parameters.max_time_in_seconds = 30.0; status2 = solver2.Solve(model2)
    ...
    with st.spinner("パターン3 (平準化重視) を探索中..."):
        ...
        solver3 = cp_model.CpSolver(); solver3.parameters.max_time_in_seconds = 30.0; status3 = solver3.Solve(model3)
...
def display_result(result_data, staff_info, event_units, year, month):
    ...
    with st.expander("業務負荷の詳細分析"):
        ...
        for d in weekdays:
            ...
            daily_units = sum(
                int(staff_info[sid]['1日の単位数']) * (0.5 if sid in half_day_staff_ids else 1)
            ...
            load_analysis.append({
                ...
                '出勤人数': len(work_staff_ids) - len(half_day_staff_ids) * 0.5,
                '提供単位数': daily_units,
                '特別業務': event_units.get(d, 0),
                '残余単位数': daily_units - event_units.get(d, 0)
            })
        
        load_df = pd.DataFrame(load_analysis)
        if len(load_df) > 0:
            avg_residual = load_df['残余単位数'].mean()
            load_df['平均との差'] = load_df['残余単位数'] - avg_residual
            st.dataframe(load_df, hide_index=True)
            st.write(f"平均残余単位数: {avg_residual:.1f}")
...
with st.expander("▼ 各種パラメータを設定する", expanded=True):
    ...
    with c3:
        ...
        st.markdown("---")
        # ★ 新しく追加: 平滑化強度の調整
        unit_penalty_multiplier = st.slider(
            "業務負荷平滑化の強度", 
            min_value=1.0, 
            max_value=10.0, 
            value=5.0, 
            step=0.5,
            help="値を大きくすると、日々の業務量の差が小さくなります。デフォルトは5.0です。"
        )
    st.markdown("---")
...
with st.expander("現在のルール一覧を表示"):
    ...
    - 🔵 **S6:** 平日の **業務負荷（残余単位数）** を平坦にする (基本: {round(10 * unit_penalty_multiplier)}、平準化重視: {round(20 * unit_penalty_multiplier)})
    - 🔵 **S7:** 平日の **職種ごと人数** を平坦にする (基本: {round(5 * unit_penalty_multiplier)}、平準化重視: {round(15 * unit_penalty_multiplier)})
    
    #### 新しく追加された平滑化機能
    - 🆕 **極端に少ない出勤の防止:** 平均の70%を下回る場合は追加ペナルティ (100点)
    - 🆕 **最小出勤人数の確保:** 全職員の40%以上の出勤を推奨 (違反時50点)
    - 🆕 **連続日の変動抑制:** 隣接する平日間の単位数差を最小化 (差分×10点)
    """)
if create_button:
    if staff_file is not None and requests_file is not None:
        try:
            ...
            is_feasible, results, message, requests_must_work = solve_three_patterns(
                ...
                unit_penalty_multiplier  # 新しいパラメータ
            )
            st.success(message)
            if is_feasible:
                staff_info = staff_df.set_index('職員番号').to_dict('index')
