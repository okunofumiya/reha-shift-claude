...
# ★★★ バージョン情報 ★★★
APP_VERSION = "proto.1.5" # パフォーマンス調整機能追加
...
def _create_schedule_df(shifts_values, staff, days, staff_df, requests_x, requests_tri, requests_paid, requests_special, requests_must_work):
    ...
    for s in staff:
        row = []; 
        s_requests_x = requests_x.get(s, []); s_requests_tri = requests_tri.get(s, []); s_requests_paid = requests_paid.get(s, []); s_requests_special = requests_special.get(s, []); s_requests_must = requests_must_work.get(s, [])
        for d in days:
...
def _calculate_penalty_breakdown(shifts_values, params):
    ...
    unit_penalty = 0; staff_penalty = 0; consecutive_penalty = 0
    ...
    if params.get('suppress_consecutive_diff', False):
        for i in range(len(params['weekdays']) - 1):
            d1, d2 = params['weekdays'][i], params['weekdays'][i + 1]
            units_d1 = sum(shifts_values.get((s, d1),0) * int(params['staff_info'][s]['1日の単位数']) * (0.5 if s in params['requests_half'].get(s, []) and d1 in params['requests_half'][s] else 1.0) for s in params['staff'])
            units_d2 = sum(shifts_values.get((s, d2),0) * int(params['staff_info'][s]['1日の単位数']) * (0.5 if s in params['requests_half'].get(s, []) and d2 in params['requests_half'][s] else 1.0) for s in params['staff'])
            consecutive_penalty += abs(units_d1 - units_d2)
        breakdown['S8: 隣接日変動抑制'] = round(consecutive_penalty * 10)
...
def solve_three_patterns(staff_df, requests_df, year, month, 
    ...
    def build_model(add_distance_constraint=False, base_solution=None, high_flat_penalty=False):
        ...
        unit_penalty_weight = round(4 * unit_penalty_multiplier) if high_flat_penalty else round(2 * unit_penalty_multiplier)
        staff_penalty_weight = round(3 * unit_penalty_multiplier) if high_flat_penalty else round(1 * unit_penalty_multiplier)
        ...
        total_expected_units = sum(int(staff_info[s]['1日の単位数']) * (len(weekdays) / (len(weekdays)+len(sundays))) * (len(days) - 9 - len(requests_paid.get(s,[])) - len(requests_special.get(s,[]))) for s in staff)
        ...
        if suppress_consecutive_diff:
            for i in range(len(weekdays) - 1):
                ...
                units_d1 = sum(shifts[(s, d1)] * int(staff_info[s]['1日の単位数']) * (0.5 if s in st.session_state.requests_half.get(s, []) and d1 in st.session_state.requests_half[s] else 1.0) for s in staff)
                units_d2 = sum(shifts[(s, d2)] * int(staff_info[s]['1日の単位数']) * (0.5 if s in st.session_state.requests_half.get(s, []) and d2 in st.session_state.requests_half[s] else 1.0) for s in staff)
    ...
    total_expected_units_for_avg = sum(int(staff_info[s]['1日の単位数']) * (len(weekdays) / (len(weekdays)+len(sundays))) * (len(days) - 9 - len(requests_paid.get(s,[])) - len(requests_special.get(s,[]))) for s in staff)
    params['avg_residual_units'] = (total_expected_units_for_avg - sum(event_units.values())) / len(weekdays) if weekdays else 0
    params['target_staff_weekday'] = {job: sum(len(days) - 9 - len(requests_paid.get(s,[])) - len(requests_special.get(s,[])) for s in members) / len(weekdays) if weekdays else 0 for job, members in params['job_types'].items() if members}
...
with st.expander("現在のルール一覧を表示"):
    ...
    - 🔵 **S6:** 平日の **業務負荷（残余単位数）** を平坦にする (ペナルティ: {round(2*unit_penalty_multiplier)}、平準化重視: {round(4*unit_penalty_multiplier)})
    - 🔵 **S7:** 平日の **職種ごと人数** を平坦にする (ペナルティ: {round(1*unit_penalty_multiplier)}、平準化重視: {round(3*unit_penalty_multiplier)})
    - 🔵 **S8:** 【検証用】隣接日の変動を抑制する (ペナルティ: 10)
    """)
if create_button:
